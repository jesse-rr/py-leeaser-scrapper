...
def parse_page(url: str, html: str) -> dict:
    try:
        ...
        blob = url.split("/")[-1]
        data = {"title": blob.replace("-", " ").title(), "url": url, "author": None, "genres": [], "tags": [], "status": None, "description": None, "cover_url": f"https://novelbin.com/media/novel/{blob}.jpg"}
        ...
        for li in info_items:
            ...
            if "author" in header_text:
                ...
                data["genres"] = [a.get_text(strip=True) for a in li.select("a")]
            elif "status" in header_text:
                a = li.select_one("a")
                if a:
                    data["status"] = a.get_text(strip=True)
            elif "tag" in header_text:
                data["tags"] = [a.get_text(strip=True) for a in li.select("a")]
        ...
        return {"title": url.split("/")[-1].replace("-", " ").title(), "url": url, "author": None, "genres": [], "tags": [], "status": None, "description": None, "cover_url": f"https://novelbin.com/media/novel/{url.split('/')[-1]}.jpg", "error": f"Parse error: {str(e)}"}
